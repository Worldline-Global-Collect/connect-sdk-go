...
func (m *fakeMarshaller) Unmarshal(data string, v interface{}) error {
	...
	event, ok := v.(*domain.WebhooksEvent)
	if ok {
...
func TestUnmarshalAPIVersionMismatch(t *testing.T) {
	...
	ClearInMemorySecretKeys()
	_ = StoreInMemorySecretKey(testKeyID, testSecretKey)

	helper, err := V1().
		NewHelperBuilder(keyStore).
		WithMarshaller(&fakeMarshaller{}).
		Build()
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalNoSecretKeyAvailable(t *testing.T) {
	...
	helper, err := V1().NewHelper(keyStore)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalMissingHeaders(t *testing.T) {
	...
	var requestHeaders []communication.Header
...
func TestUnmarshalInvalidSecretKey(t *testing.T) {
	keyStore := InMemorySecretKeyStore()
	ClearInMemorySecretKeys()
	_ = StoreInMemorySecretKey(testKeyID, "1"+testSecretKey)
...
func init() {
	signatureHeader, _ := communication.NewHeader("X-GCS-Signature", testSignature)
	keyHeader, _ := communication.NewHeader("X-GCS-KeyId", testKeyID)
