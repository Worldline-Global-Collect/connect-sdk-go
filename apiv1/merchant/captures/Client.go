...
//     or the service that you're trying to reach is temporary unavailable (HTTP status code 500, 502 or 503)
//   * APIError if the Worldline Global Collect platform returned any other error
func (c *Client) Disputes(captureID string, context *communicator.CallContext) (domain.DisputesResponse, error) {
	...
	uri, err := c.apiResource.InstantiateURIWithContext("/v1/{merchantId}/captures/{captureId}/disputes", pathContext)
	if err != nil {
		return resultObject, err
	}
...
//     or the service that you're trying to reach is temporary unavailable (HTTP status code 500, 502 or 503)
//   * APIError if the Worldline Global Collect platform returned any other error
func (c *Client) Dispute(captureID string, body domain.CreateDisputeRequest, context *communicator.CallContext) (domain.DisputeResponse, error) {
	...
	uri, err := c.apiResource.InstantiateURIWithContext("/v1/{merchantId}/captures/{captureId}/dispute", pathContext)
	if err != nil {
		return resultObject, err
	}
