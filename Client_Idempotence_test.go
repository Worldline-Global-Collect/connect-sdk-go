...
var idempotenceDuplicateFailureJSON = `{
...
func createClient(socketTimeout, connectTimeout time.Duration, port int) (*Client, error) {
	connection, err := communicator.NewDefaultConnection(socketTimeout, connectTimeout, 30*time.Second, 50*time.Second, 500, nil)
	if err != nil {
		return nil, err
	}

	authenticator, err := v1hmac.NewAuthenticator("apiKey", "secret")
	if err != nil {
		return nil, err
	}
	...
	comm, err := communicator.NewCommunicator(endPoint, connection, authenticator, metadataProvider, marshaller)
	if err != nil {
		return nil, err
	}

	return NewClient(comm)
}

func createRequest() domain.CreatePaymentRequest {
	...
	billingAddress := domain.NewAddress()
	billingAddress.CountryCode = NewString("CA")
	customer.BillingAddress = billingAddress

	order.Customer = customer

	cardPaymentMethodSpecificInput := domain.NewCardPaymentMethodSpecificInput()
...
func TestIdempotenceFirstRequest(t *testing.T) {
	...
	response, err := client.V1().Merchant("20000").Payments().Create(request, context)
	if err != nil {
		t.Fatalf("%v: %v", logPrefix, err)
	}
...
func TestIdempotenceDuplicateRequest(t *testing.T) {
	...
	listener, sl, client, err := createTestEnvironment(
		"/v1/20000/payments",
		createRecordRequest(http.StatusConflict, idempotenceDuplicateFailureJSON, responseHeaders, requestHeaders))
	if err != nil {
		t.Fatalf("%v: %v", logPrefix, err)
	}
	defer func(listener net.Listener, sl *stoppableListener, client *Client) {
